 ...
 */
package com.yunfei.wh.ui.scale;
...
public class GestureImageViewTouchListener implements OnTouchListener {
			...
			if (!tapDetector.onTouchEvent(event)) {
				...
				if (event.getAction() == MotionEvent.ACTION_UP) {
					...
					if (event.getPointerCount() > 1) {
						...
						image.getParent().requestDisallowInterceptTouchEvent(true);// 屏蔽父控件(LiaoBo 2015.7.3修正，兼容在多点触控下横向拉伸和fu)
					} else {
						if (!touched) {
							touched = true;
							last.x = event.getX();
	...
	protected void boundCoordinates() {
		if (next.x < boundaryLeft) {// 图片超出左边界
			next.x = boundaryLeft;
			image.getParent().requestDisallowInterceptTouchEvent(false);// 图片已经拖出到边界，让父控件处理滑动
		} else if (next.x > boundaryRight) {// 图片超出右边界
