...
#define LOG_I(...) __android_log_print(ANDROID_LOG_INFO, "EasyAR", __VA_ARGS__)
#else
...
//            target.load(jstr.c_str(), EasyAR::kStorageAssets | EasyAR::kStorageJson);
            target.load(jstr.c_str(), EasyAR::kStorageJson | EasyAR::kStorageAssets);
